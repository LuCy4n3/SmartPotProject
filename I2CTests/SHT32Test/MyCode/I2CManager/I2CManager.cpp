...
*/
i2c_error_t I2C_0_do_transfer(uint8_t addr,uint8_t *data, uint8_t countToWrite,uint8_t countToRead)
{
	...
	if(countToRead>0)
	I2C_0_set_data_complete_callback(I2C_0_read_handler, &readDescriptor);
	...
	I2C_0_set_buffer((void *)data, countToWrite);
...
i2c_error_t I2CManagerClass::WriteRegisterFromInternalBuffer(uint8_t address, uint8_t countToWrite,uint8_t countToRead){
	...
	i2c_error_t returnVal = I2C_0_do_transfer(address,WriteBuffer,countToWrite,countToRead);
	for(uint8_t i = 0;i<8;i++)
	{
...
i2c_error_t I2CManagerClass::ReadAmount(uint8_t address, uint8_t amount)
{
	...
	I2C_0_open (address);
	I2C_0_set_buffer(ReadBuffer, amount);
	I2C_0_master_read();
